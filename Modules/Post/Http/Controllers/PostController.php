...
class PostController extends Controller
{
    ...
    public function createArticle()
    {
        ...
        $categories     = Category::with('childrenRecursive')->where('parent_id', 0)->where('language', \App::getLocale() ?? settingHelper('default_language'))->get();
...
//        dd($request->all());
    ...
    {
         $posts     = Post::orderBy('id','desc')->where('posts.slider',1)->with('image','category','user')->paginate('15');

        return view('post::slider_posts',compact('posts'));
    }

    public function featuredPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.featured',1)->with('image','category','user')->paginate('15');

        return view('post::featured_posts',compact('posts'));
    }

    public function breakingPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.breaking',1)->with('image','category','user')->paginate('15');

        return view('post::breaking_posts',compact('posts'));
    }

    public function recommendedPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.recommended',1)->with('image','category','user')->paginate('15');

        return view('post::recommended_posts',compact('posts'));
    }

    public function editorPicksPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.editor_picks',1)->with('image','category','user')->paginate('15');

        return view('post::editor_picks',compact('posts'));
    }

    public function pendingPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.status',0)->with('image','category','user')->paginate('15');
        return view('post::pending_posts',compact('posts'));
    }

    public function submittedPosts(){
         $posts     = Post::orderBy('id','desc')->where('posts.submitted',1)->with('image','category','user')->paginate('15');
    ...
    public function editPost($type,$id){
        ...
        $categories       = Category::with('childrenRecursive')->where('parent_id', 0)->where('language',$post->language)->get();
        ...
        elseif($type == 'trivia-quiz'):
            $post           = Post::where('id',$id)->with(['image','video','videoThumbnail','category','quizResults'])->first();
...
//            dd($quiz_questions);
    ...
    public function filterPost(Request $request){
        ...
        $posts = Post::where('language', 'like', '%' . $request->language .'%')
                ...
                ->orderBy('id','desc')
                ->with('image','video','category','user')
                ->paginate('15');
                // return $search_query;
